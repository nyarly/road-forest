...
app = LogicalConstruct::WebmachineResolver::WebApplication.new
...
module LogicalConstruct
  class WebmachineResolver
    class WebApplication < RoadForest::Application
      include RoadForest::Resources

      def routes
        [
          :root,              [],                       bundle_model(ReadOnly,  Models::Navigation)
          :needs,             ["needs"],                bundle_model(List,      Models::NeedList)
          :resolved_needs,    ["resolved-needs"],       bundle_model(List,      Models::ResolvedNeedList)
          :unresolved_needs,  ["unresolved-needs"],     bundle_model(List,      Models::UnresolvedNeedList)
          :need,              ["needs",'*'],            bundle_model(LeafItem,  Models::Need)
          :status,            ["provisioning-status"],  bundle_model(LeafItem,  Models::Status)
          :file,              ["needs", "file", "*"],   bundle_model(File,      Models::NeedFile)
        ]
      end
    end
